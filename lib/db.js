  ...
  try {
    ...
    return await LeaderBoard.findAll({
      ...
      order: [
        ['time', 'ACS'],
        ['times_played', 'ACS'],
      //  it also needs to be in order of user somehow, needs to discuss that as well
      ]
    });
  } catch (error) {
...
// what is the concept of entry without gameID
// no concept of leaderboard entry without gameID
module.exports.saveCurrentEntry = async ({user, time, timesPlayed}) => {
  try {
    const {LeaderBoard} = await connectToDatabase();
    return await LeaderBoard.create({
      // add user reference and time will be added by ORM
...
// will provide the referral link to the current user
// this referral link will then be used to create user send from
// frontend and will be saved in the users tale, generated once
// will need to get from cognito users, don't know RN
module.exports.getMyReferralLink = async (user) => {
